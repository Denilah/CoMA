...
from sample_generator_new import train_tokenize_function
...
def main():
    ...
    with torch_distributed_zero_first(global_rank):
        raw_train_datasets = load_dataset(
            "json", data_files=data_args.train_file, cache_dir=model_args.cache_dir, split="train"
        )

        raw_eval_datasets = load_dataset(
            "json", data_files=data_args.validation_file, cache_dir=model_args.cache_dir, split="train"
        )
        ...
        if model_args.use_flash_attention:
            ...
            train_data = raw_train_datasets.map(
            ...
            val_data = raw_eval_datasets.map(
                       train_tokenize_function,
                       batched=True,
                       num_proc=1,
    ...
    print_rank_0(
        "Train dataset size: {}".format(len(train_data)), log_file, global_rank
    )
    print_rank_0(
        "Train tokenized example: {}".format(train_data[0]), log_file, global_rank
    )
    print_rank_0(
        "Eval dataset size: {}".format(len(val_data)), log_file, global_rank
    )
    print_rank_0(
        "Eval tokenized example: {}".format(val_data[0]), log_file, global_rank
    )
